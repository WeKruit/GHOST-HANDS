# PRD: Mastra Orchestration for GHOST-HANDS (V5.1)
...
V5 is the strongest draft to date, but still has blockers:

1. Step I/O schemas do not compose across `.then()` and `.branch()`.
2. Queue wake strategy is incomplete in pg-boss dispatch mode.
3. Resume route returns status semantics that conflict with DB state.
4. Snippets still use non-existent APIs (`remainingBudget()`).
5. Workflow output does not cleanly map to `TaskResult` used by current finalization.

This V5.1 resolves those blockers and narrows scope where needed.
...
V5 had step shape mismatch. V5.1 uses a single composable state object.
...
const workflowState = z.object({
  ...
  status: z.enum(['running', 'suspended', 'completed', 'failed']).default('running'),
});
```
...
3. `execute_handler`
   - executes resolved handler path and writes `handler.taskResult`
...
export function buildSteps(rt: RuntimeContext) {
  const checkBlockers = createStep({
    ...
    execute: async ({ inputData, resumeData, suspend }) => {
      ...
      // ── Fresh blocker check ──
      const blockers = await rt.adapter.observeWithBlockerDetection?.('Check for blockers');
      const blocker = blockers?.find(b => b.confidence > 0.6);

      if (blocker) {
        // Side effects before suspend
        await pauseJob(rt.supabase, state.jobId);
        await rt.logEvent('blocker_detected', { type: blocker.type });
        await sendNeedsHumanCallback(rt, state, blocker);

        state.hitl.blocked = true;
        state.hitl.blockerType = blocker.type;
  ...
  const executeHandler = createStep({
    ...
    execute: async ({ inputData }) => {
      ...
      state.handler.success = result.success;
...
4. Run workflow with composable `workflowState`.
5. If suspended: return without running completion finalization.
6. If completed:
   - require `handler.taskResult` (or cookbook-success synthetic equivalent)
   - pass into existing finalization path with minimal refactor.

Important: avoid duplicating finalization logic. Extract a shared helper:
...
interface FinalizationInput {
  ...
  costTracker: CostTracker;
  progress: ProgressTracker;
  taskResult: TaskResult;           // from handler or cookbook-success synthetic
  finalMode: string;                // 'cookbook' | 'magnitude' | 'smart_apply' | etc.
...
/**
 * Shared finalization: screenshot, session save, DB update, callback, cost record.
 * Called from both legacy path and mastra path in JobExecutor.
 */
export async function finalizeJob(input: FinalizationInput): Promise<void> {
  // 1. Take final screenshot + upload
  // 2. Save browser session (if sessionManager exists)
  // 3. Update gh_automation_jobs (status, result_data, cost, etc.)
  // 4. Log job_completed event
  // 5. Record cost via CostControlService
  // 6. Send callback to VALET
  // (Extracted from JobExecutor lines ~637-718)
}
```

This helper is called identically from both the existing legacy path and the new mastra path, ensuring behavior parity.
...
3. Set metadata flags:
   - `resume_requested = true`
   - `resume_nonce = <uuid>`
4. Set status to resumable queued state for the active dispatch mode.
5. Return status consistent with DB state (`pending` or `queued`, not `running`).
...
- If `queue` mode is active and no API-side enqueue capability exists, reject mastra resume with explicit error and guidance.
...
1. `018_mastra_execution_mode_and_resume_flags.sql` (if needed)
2. `019_callback_dedupe.sql`
3. `020_mastra_rls_and_retention.sql` (or combined, depending on policy)
...
5. Extract shared finalization helper from JobExecutor.
...
- [ ] Extract `workers/finalization.ts` from JobExecutor
- [ ] Add mastra branch in `JobExecutor.execute()` (resume discriminator + workflow execution)
...
- [ ] Add migration `018_callback_dedupe.sql`
- [ ] Add migration `019_mastra_rls_and_retention.sql`
