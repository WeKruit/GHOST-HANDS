...
export async function handleJobListing(adapter: MinimalAdapter, pageState: PageState): Promise<void> {
  const logger = getLogger();
...
export async function handleVerificationCode(adapter: MinimalAdapter): Promise<void> {
  const logger = getLogger();
...
export async function handlePhone2FA(adapter: MinimalAdapter): Promise<void> {
  const logger = getLogger();
  const currentUrl = await adapter.getCurrentUrl();
...
export async function handleExperiencePage(
  ...
  // Date helpers shared by work experience and education sections
...
Do NOT click any "Add" buttons. Do NOT touch any other sections. Do NOT interact with fields under any other "Work Experience" heading.`;
  ...
  // ==================== DELETE EXTRA WORK EXPERIENCE ENTRIES ====================
  // Workday may pre-populate or require more entries than the user has.
  // Count visible entry headings beyond what we filled and delete them.
  const extraWorkEntries = await adapter.page.evaluate(`
    (() => {
      var headings = document.querySelectorAll('h2, h3, h4, h5');
      var count = 0;
      for (var i = 0; i < headings.length; i++) {
        var text = (headings[i].textContent || '').trim();
        if (/^Work Experience\\s*\\d+/i.test(text)) count++;
      }
      return count;
    })()
  `) as number;

  if (extraWorkEntries > experiences.length) {
    const toDelete = extraWorkEntries - experiences.length;
    logger.info('Deleting extra work experience entries', { extraEntries: toDelete, filled: experiences.length, total: extraWorkEntries });

    const deleteWorkPrompt = buildExperienceEntryPrompt(
      `There are ${extraWorkEntries} Work Experience entries on this page but only ${experiences.length} should exist. ` +
      `Delete the LAST ${toDelete} Work Experience entr${toDelete === 1 ? 'y' : 'ies'} (the empty/unfilled ones). ` +
      `For each extra entry, find its Delete button (trash icon, "Delete", or "Remove" button) and click it. ` +
      `If a confirmation dialog appears, confirm the deletion. ` +
      `Do NOT delete entries ${experiences.length > 0 ? `1 through ${experiences.length}` : ''} — those are already filled.` +
      `\nDo NOT touch Education, Skills, or any other section.`
    );

    await adapter.act(deleteWorkPrompt);
    llmCallCount++;
    await adapter.page.waitForTimeout(1000);
  }
  ...
  for (let i = 0; i < educations.length; i++) {
    ...
    // Parse dates for MM/YYYY instruction
    const parseEduDate = (raw: string | undefined, fallback: Date) => {
      if (!raw) return fmtDate(fallback);
      const parts = raw.split('-');
      return parts.length >= 2 ? `${parts[1]}/${parts[0]}` : raw;
    };
    const eduFromDate = parseEduDate(edu.start_date, yesterday);
    const eduToDate = parseEduDate(edu.end_date, today);

    const gpaLine = edu.gpa ? `\n  Overall Result (GPA): ${edu.gpa}` : '';
    const eduToDateLine = edu.end_date
      ? `\n  To date: ${eduToDate} — click on the "MM" text, then type "${eduToDate.replace('/', '')}" as continuous digits.`
      : '';
...
Fill ALL of these fields (scroll down if needed to find them):
  ...
  From date: ${eduFromDate} — click on the "MM" text, then type "${eduFromDate.replace('/', '')}" as continuous digits.${eduToDateLine}
...
Do NOT click any "Add" buttons. Do NOT touch any other sections. Do NOT interact with fields under any other "Education" heading.`;
  ...
  // ==================== DELETE EXTRA EDUCATION ENTRIES ====================
  const extraEduEntries = await adapter.page.evaluate(`
    (() => {
      var headings = document.querySelectorAll('h2, h3, h4, h5');
      var count = 0;
      for (var i = 0; i < headings.length; i++) {
        var text = (headings[i].textContent || '').trim();
        if (/^Education\\s*\\d+/i.test(text)) count++;
      }
      return count;
    })()
  `) as number;

  if (extraEduEntries > educations.length) {
    const toDelete = extraEduEntries - educations.length;
    logger.info('Deleting extra education entries', { extraEntries: toDelete, filled: educations.length, total: extraEduEntries });

    const deleteEduPrompt = buildExperienceEntryPrompt(
      `There are ${extraEduEntries} Education entries on this page but only ${educations.length} should exist. ` +
      `Delete the LAST ${toDelete} Education entr${toDelete === 1 ? 'y' : 'ies'} (the empty/unfilled ones). ` +
      `For each extra entry, find its Delete button (trash icon, "Delete", or "Remove" button) and click it. ` +
      `If a confirmation dialog appears, confirm the deletion. ` +
      `Do NOT delete entries ${educations.length > 0 ? `1 through ${educations.length}` : ''} — those are already filled.` +
      `\nDo NOT touch Work Experience, Skills, or any other section.`
    );

    await adapter.act(deleteEduPrompt);
    llmCallCount++;
    await adapter.page.waitForTimeout(1000);
  }
