#!/usr/bin/env bun
/**
 * Submit Google Careers Application Test Job
 *
 * Creates a smart_apply job in the database that a running worker will pick up.
 * Uses the generic platform config (no Google-specific workflows).
 *
 * Prerequisites:
 *   1. Worker running: bun run worker -- --worker-id=<name>
 *
 * Usage:
 *   npx tsx --env-file=.env src/scripts/apply-google.ts                        # any worker picks it up
 *   npx tsx --env-file=.env src/scripts/apply-google.ts --worker-id=adam        # only worker "adam" picks it up
 *   npx tsx --env-file=.env src/scripts/apply-google.ts --url=https://...       # custom Google Careers URL
 */
...
const DEFAULT_GOOGLE_URL =
  'https://www.google.com/about/careers/applications/jobs/results/91161696034267846-software-engineer-iii-aiml-youtube-ads-ml';

const TEST_USER_ID = process.env.GH_TEST_USER_ID || '00000000-0000-0000-0000-000000000001';

const TEST_GOOGLE_PROFILE = {
  first_name: 'Happy',
  last_name: 'Wu',
  ...
  experience: [
    {
      ...
      start_date: '2023-06',
      end_date: '',
      description: 'Full-stack development on browser automation platform.',
    },
  ],
  skills: ['Python', 'TypeScript', 'Go', 'Distributed Systems', 'Cloud Infrastructure'],
...
const TEST_QA_OVERRIDES: Record<string, string> = {
  ...
  'Have you previously worked for Google?': 'No',
...
async function main() {
  ...
  console.log('\nCreating Google Careers application job...\n');

  const inputData = {
    user_data: TEST_GOOGLE_PROFILE,
  ...
  const taskDescription = [
    'Fill out the entire Google job application.',
  ...
  try {
    ...
    `,
      [
        ...
        TEST_USER_ID,
        targetWorkerId,
        JSON.stringify(['google', 'test']),
      ],
    );

    const job = result.rows[0];
    console.log('Google Careers application job created!\n');
    console.log(`   Job ID:     ${job.id}`);
    console.log(`   Status:     ${job.status}`);
    console.log(`   URL:        ${job.target_url}`);
    console.log(`   Applicant:  ${TEST_GOOGLE_PROFILE.first_name} ${TEST_GOOGLE_PROFILE.last_name}`);
    console.log(`   Email:      ${TEST_GOOGLE_PROFILE.email}`);
    ...
    console.log('\n   The worker will:');
    console.log('     1. Navigate to the Google Careers job listing');
    console.log('     2. Click Apply');
    console.log('     3. Sign in with Google if prompted');
