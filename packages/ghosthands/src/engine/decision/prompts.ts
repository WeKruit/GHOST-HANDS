...
export const PLATFORM_GUARDRAILS: Record<string, string> = {
  workday: [
    ...
    'Never use Google SSO or any other SSO provider on Workday-host pages. Stay on the native email/password path only.',
    'If "Apply Manually" is visible after clicking Apply, choose that first before any field-filling.',
...
export function buildSystemPrompt(profileSummary: string, platformGuardrails: string): string {
  ...
  const prompt = [
    ...
    '- Leave optional fields empty when the applicant profile does not provide an explicit value.',
    ...
    '- On Workday, auth flows are native only: never choose an SSO path.',
