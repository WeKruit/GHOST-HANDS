...
import type { PlatformConfig, PageState, PageType, ScannedField, ScanResult } from './platformTypes';
...
IMPORTANT: Many job sites show a "Submit" button on EVERY page — this does NOT mean you are on the review page. Only classify as "review" if the page is purely a read-only summary with no fields to fill.`;
  ...
  async classifyByDOMFallback(adapter: BrowserAutomationAdapter): Promise<PageType> {
    return adapter.page.evaluate(() => {
      ...
      if (allText.includes('thank you for applying') || allText.includes('thank you for your application') || allText.includes('application received') || allText.includes('successfully submitted') || allText.includes('application has been submitted')) return 'confirmation' as PageType;
