...
describe('CostTracker mode tracking', () => {
  ...
  test('getSnapshot() includes magnitudeSteps=0 by default', () => {
    const snap = tracker.getSnapshot();
    expect(snap.magnitudeSteps).toBe(0);
    expect(snap.mode).toBe('magnitude');
  });

  test('recordModeStep() increments magnitudeSteps', () => {
    tracker.recordModeStep();
    const snap = tracker.getSnapshot();
  ...
  test('multiple recordModeStep calls accumulate correctly', () => {
    ...
    const snap = tracker.getSnapshot();
    expect(snap.magnitudeSteps).toBe(3);
  });
});
...
describe('ProgressTracker mode tracking', () => {
  ...
  test('setExecutionMode sets mode in snapshot', () => {
